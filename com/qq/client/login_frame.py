# coding=utf-8
# 代码文件：peace\com\qq\client\login_frame.py
...
from com.qq.client.reg_frame import RegFrame
...
class LoginFrame(MyFrame):
    def __init__(self):
        ...
        # 创建顶部图片
        topimage = wx.Bitmap('resources\images\顶部图.png', wx.BITMAP_TYPE_PNG)
        # topimage = wx.Bitmap('resources\images\qq2.jpg',wx.BITMAP_TYPE_JPEG)
        ...
        # st = wx.Button(middlepanel, label='忘记密码?')
        ...
        self.Bind(wx.EVT_BUTTON, self.reg_btn_onclick, reg_btn)
        # 创建水平Box布局Hbox对象
        hbox = wx.BoxSizer(wx.HORIZONTAL)
        hbox.Add(reg_btn, 1, wx.CENTER | wx.ALL | wx.EXPAND, border=10)
    ...
    def okb_btn_onclick(self, event):
        ...
        if user is not None:
            logger.info('登录成功.')
            next_frame = FriendsFrame(user)     # 界面跳转
            next_frame.Show()
            self.Hide()
        else:
            logger.info('登录失败')
